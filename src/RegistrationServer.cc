...
void RegistrationServer::initialize()
{
    ...
    // par("processingTime") returns seconds; store as seconds, display as ms
    processingTime        = par("processingTime").doubleValue();
    // par("serverCapacity") returns Mbps directly (NED unit is Mbps)
    serverCapacity        = par("serverCapacity").doubleValue();
    ...
    EV << "[SERVER] Registration Server initialized. "
       ...
       << ", ProcessingTime=" << processingTime * 1000.0 << "ms\n";
}
